<template>
  <div class="container">
    <h1 class="page-title flex-center">
      ...
      <table class="table">
        ...
        <tbody class="table-body">
          <template v-if="initialLoading">
            <tr v-for="i in itemsPerPage" :key="i" class="table-loading">
              <td :colspan="localHeaders.length" class="table-empty">
                <div v-if="i === Math.ceil(itemsPerPage / 2)" class="flex-center h-full">
          ...
          <template v-else-if="posts && posts.length">
            <tr v-for="post in posts" :key="post.id" class="table-row" @click="goToPostDetail(post.id)">
              <td v-for="header in localHeaders" :key="header.key" class="table-cell" :class="header.class">
                <template v-if="header.key === 'title'">
                  <span class="block sm:hidden text-xs text-muted">
                    <Icon icon="mdi:account" class="inline mr-1" />{{ post.author }} | 
                    <Icon icon="mdi:calendar" class="inline mr-1" />{{ formatDate(post.createdAt) }}
                  </span>
                  <span class="text-truncate" :class="{ 'pl-5': post.parentId }">
...
const props = defineProps({
  ...
  adminBoardType: {
    type: String, // 실제 게시판 유형 (freeboard, humor, qna)
    required: true
  },
  boardTitle: {
    type: String,
...
const { data: posts, error, refresh } = await useAsyncData(props.apiEndpoint, async function() {
  try {
    const response = await $fetch(props.apiEndpoint, {
      params: {
        ...
        boardType: props.adminBoardType // 새롭게 추가된 prop
      }
    })
  ...
  watch: [currentPage, itemsPerPage, searchParams, sortColumn, sortOrder, () => props.adminBoardType],
  server: false
})
 ...
 * 관리자 페이지 경로에 맞게 URL을 구성합니다.
 * @param {number} postId - 이동할 게시글의 ID.
 */
function goToPostDetail(postId) {
  router.push(`/adminpage/posts/view?boardType=${props.adminBoardType}&id=${postId}`)
}
