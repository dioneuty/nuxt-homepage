...
onMounted(async () => {
  if (route.query.id) {
    ...
    const { data, error } = await useFetch(`/api/wiki?id=${route.query.id}`)
    if (error.value) {
...
async function submitWiki() {
  const url = isEditing.value ? `/api/wiki?id=${route.query.id}` : '/api/wiki'
  ...
  try {
    ...
    openModal('오류', `Wiki 페이지 ${isEditing.value ? '수정' : '작성'}에 실패했습니다.`)
  }
}
</script>