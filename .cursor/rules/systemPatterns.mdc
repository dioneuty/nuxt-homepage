...
### 시스템 아키텍처 뷰
아래 다이어그램은 프로젝트의 주요 구성 요소와 데이터 흐름을 보여줍니다.
...
graph TD
    ...
    C --> D[페이지 컴포넌트 데이터 요청]
    D --> E[서버 API 처리]
    E -- 데이터베이스 요청 --> F[Prisma/DB]
    F --> G[데이터베이스 응답]
    G --> H[서버 API 응답]
    H --> I[페이지 컴포넌트 UI 렌더링]
    I --> J{홈페이지 표시 완료}
    J --> K[사용자 상호작용 시작]
    K --> L{내비게이션 클릭/검색/로그인/컨텐츠 클릭}
    L --> M[관련 페이지로 이동]
    M --> N[새로운 페이지 흐름 시작]
    N --> A
```
...
### 게시판 및 다른 페이지 기능 흐름
게시판 및 다른 콘텐츠 페이지(블로그, 갤러리 등)의 일반적인 기능 흐름은 다음과 같습니다.
...
graph TD
    ...
    J --> K[목록 페이지 사용자 상호작용 시작]
    K --> L{페이지네이션/필터/정렬/검색/아이템 클릭/글쓰기}

    subgraph 상세 보기
        L -- 아이템 클릭 --> M[상세 페이지 접속]
        M --> N[상세 데이터 로딩 시작]
        N --> O[상세 페이지 컴포넌트 상세 데이터 요청]
        O --> P[서버 API 처리]
        P --> Q[Prisma]
        Q --> R[데이터베이스 상세 응답]
        R --> S[서버 API 상세 응답]
    ...
    U --> V[상세 페이지 사용자 상호작용 시작]
    V --> W{댓글 작성/조회/수정/삭제/목록으로 돌아가기}

    subgraph 콘텐츠 작성/수정
        L -- 글쓰기 --> X[작성/수정 페이지 접속]
        X --> Y[콘텐츠 작성/제출]
        Y --> Z[서버 API 처리]
        Z --> AA[Prisma]
        AA --> BB[데이터베이스 저장/수정 응답]
        BB --> CC[서버 API 저장/수정 응답]
        CC --> DD{목록 또는 상세 페이지로 리다이렉션}
    end
...
- **`pages/adminpage/gallery/index.vue` (갤러리 관리)**:
  - **역할**: 웹사이트의 갤러리 항목을 관리(조회, 생성, 수정, 삭제)하는 기능을 제공합니다.
  - **구현**: 갤러리 항목 목록을 테이블 형태로 표시하며, 검색, 필터링, 페이지네이션 기능을 지원합니다. 항목 추가/수정/삭제를 위한 모달 폼을 활용합니다.
  - **API 연동**: `/api/admin/gallery` 엔드포인트에 `GET`, `DELETE` 요청을 보내 갤러리 데이터를 조작합니다. 상세 조회 시 ID를 쿼리 파라미터(`?id=`)로 전달합니다.
  - **특징**: Nuxt.js 라우팅 규칙에 따라 `pages/adminpage/gallery` 디렉토리 내의 `index.vue` 파일이 해당 경로의 기본 페이지 역할을 하며, 상세 페이지는 `pages/adminpage/gallery/view.vue`로 분리되어 `id` 쿼리 파라미터로 접근합니다.

- **`pages/adminpage/gallery/view.vue` (갤러리 항목 상세)**:
  - **역할**: 특정 갤러리 항목의 상세 정보를 표시하고, 수정 및 삭제 기능을 제공합니다.
  - **구현**: 갤러리 항목의 ID를 쿼리 파라미터(`route.query.id`)를 통해 받아 해당 항목의 상세 정보를 조회합니다.
  - **API 연동**: `/api/admin/gallery` 엔드포인트에 `GET`, `PUT`, `DELETE` 요청을 통해 상세 정보를 불러오거나 수정 및 삭제 작업을 수행합니다.
...
아래 다이어그램은 시스템의 주요 유스케이스와 액터 간의 상호작용을 보여줍니다.

```mermaid
graph TD
    A[일반 사용자] --> B(페이지 탐색 및 정보 조회)
    A --> C(콘텐츠 상호작용)

    D[관리자] --> E(사이트 관리)

    subgraph "페이지 탐색 및 정보 조회"
        B --> B1(홈 페이지 조회)
        B --> B2(슬라이드/게시판/갤러리/일기예보/달력 섹션 조회)
        B --> B3(갤러리 항목 조회)
        B --> B4(방명록 조회)
        B --> B5(블로그 게시물 조회)
        B --> B6(게시판 게시물 조회)
