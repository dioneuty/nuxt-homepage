...
export default defineEventHandler(async (event) => {
  ...
  if (method === 'GET') {
    try {
      ...
      if (query.id) {
        ...
        if (isNaN(id)) {
          return handleApiError(event, 400, '유효하지 않은 ID입니다.');
        }
        ...
        if (!wiki) {
          return handleApiError(event, 404, '위키 페이지를 찾을 수 없습니다.');
        }
      ...
      handleApiError(event, 500, '위키 페이지 조회 중 오류가 발생했습니다.', error);
    }
  }
  ...
  if (['PUT', 'POST', 'DELETE'].includes(method)) {
    ...
    if (!token) {
      return handleApiError(event, 401, '인증 토큰이 없습니다.');
    }
    ...
    try {
      ...
      if (method === 'POST') {
        ...
        try {
          ...
          handleApiError(event, 500, '위키 페이지 생성 중 오류가 발생했습니다.', error);
        }
      }
      ...
      if (method === 'PUT') {
        ...
        try {
          ...
          handleApiError(event, 500, '위키 페이지 업데이트 중 오류가 발생했습니다.', error);
        }
      }
      ...
      if (method === 'DELETE') {
        try {
          ...
          handleApiError(event, 500, '위키 페이지 삭제 중 오류가 발생했습니다.', error);
        }
      }
    } catch (error) {
      // 토큰 검증 실패 시 오류 로깅 및 상세 오류 메시지 반환
      if (error.code === 'ERR_JWT_EXPIRED') {
        return handleApiError(event, 401, '토큰이 만료되었습니다.', error);
      }
