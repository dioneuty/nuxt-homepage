...
/**
 * @file 관리자 게시판 API
 * @description 관리자 게시판 게시물에 대한 CRUD (생성, 조회, 업데이트, 삭제) 작업을 처리합니다.
 *              개별 게시물 조회, 페이지네이션 및 검색/정렬을 통한 목록 조회 기능을 제공합니다.
 */
export default defineEventHandler(async (event) => {
  const method = event.node.req.method

  // GET 요청 처리: 관리자 게시물 목록 조회 또는 특정 게시물 조회
  if (method === 'GET') {
  ...
  // POST 요청 처리: 새로운 관리자 게시물을 생성합니다.
  if (method === 'POST') {
    const { title, content } = await readBody(event)
    try {
      const result = await prisma.adminBoard.create({
        data: { title, content, author: '관리자' } // 'author' 필드 추가 및 기본값 설정
      })
  ...
  // PUT 요청 처리: 특정 ID의 관리자 게시물을 업데이트합니다.
  if (method === 'PUT') {
    const { id, title, content } = await readBody(event)
    try {
      // ID를 사용하여 게시물 레코드 업데이트 (ID는 정수로 변환)
  ...
  // DELETE 요청 처리: 특정 ID의 관리자 게시물을 삭제합니다.
  if (method === 'DELETE') {
    const { id } = await readBody(event)
    try {
      // ID를 사용하여 게시물 레코드 삭제 (ID는 정수로 변환)
